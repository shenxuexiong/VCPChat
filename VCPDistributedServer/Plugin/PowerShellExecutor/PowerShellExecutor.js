...
async function executePowerShellCommand(command, executionType = 'blocking', timeout = 60000, requireAdmin = false) {
    return new Promise((resolve, reject) => {
        // 预置编码命令以确保UTF-8输出
        const fullCommand = `[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; ${command}`;

        if (requireAdmin) {
            // 最终方案：调用一个独立的Python脚本来处理UAC和GUI确认
            const pythonConfirmScript = path.join(__dirname, 'AdminConfirm.py');
            
            // 使用Base64编码命令，以安全地将其作为命令行参数传递
            const commandAsBase64 = Buffer.from(fullCommand).toString('base64');

            // 直接启动Python脚本。脚本内部会处理UAC提权。
            // 我们仍然使用detached模式，以防万一，确保最大的独立性。
            const child = spawn('python', [pythonConfirmScript, commandAsBase64], {
                detached: true,
                stdio: 'ignore',
                windowsHide: true
            });
            ...
            child.on('error', (err) => {
                // 这个错误只会在找不到python命令时触发
                reject(new Error(`无法启动Python确认脚本。请确保Python已安装并配置在系统PATH中。错误: ${err.message}`));
            });

            child.unref();

            resolve('管理员执行请求已发送给用户进行确认。请在弹出的GUI窗口中操作。');
...
async function main() {
    ...
    process.stdin.on('end', async () => {
        try {
            ...
            const requireAdmin = args.requireAdmin || false;
            ...
            if (executionType === 'background') {
                ...
                // 启动后台命令而不等待其完成
                executePowerShellCommand(command, executionType, 60000, requireAdmin)
                    .then(output => {
                ...
                // 同步执行
                const output = await executePowerShellCommand(command, executionType, 60000, requireAdmin);
